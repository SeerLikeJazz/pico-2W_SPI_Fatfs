...
先后顺序为：开电 → 等待 → SPI 初始化 → RESET → SDATAC → ID → 配置并读回 → 待机；显式启动后执行 RDATAC → START → 开启 DRDY 采集。默认 GPIO/DMA 中断都在 core 0，禁止从另一个 core 调用驱动 API。
...
| `i` | 重新复位、配置后待机；清空自动恢复预算，fatal abort 除外 |
| `t` / `h` / `n` | 切换内部测试 / 内部短接 / 正常输入后待机（再用 g 启动） |
...
6. **验证正常输入。** 先确认 H2/H5、参考电极和 INxN/INxP 的真实连接，并相应修改 SRB/BIAS 配置。输入 n 后用 g 启动，用已知受控的小信号源验证极性、幅值及共模范围。当前默认差分模式不能将悬空的 INxN 自动当成公共参考。
...
本版增加阻抗交流激励原始码模式，以及 WFC2 启停/会话保护，详见 [上位机说明](../Code/README.md)。所有 ADS 配置和启停仍在 Core 0，Core 1 通过原子邮箱通知。参考跳线、回流路径及阻抗校准仍需实机确认。
