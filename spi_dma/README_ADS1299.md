...
默认 MCLK 下，250 SPS 周期 4 ms；8 kSPS 为 125 µs；16 kSPS 为 62.5 µs。1 MHz 下单帧约 216 µs。驱动要求帧传输小于周期的 75%，为 IRQ 延迟与数据更新时间留出余量，因此默认 1 MHz 仅接受不超过标称 2000 SPS 的设置。更高采样率先调整 SPI 时钟，再用逻辑分析仪测量余量；带宽检查通过也不代表中断延迟已获得硬件保证。
