...
static DWORD WINAPI producer(LPVOID arg) {
    (void)arg;
    for(uint32_t i=0;i<200000;i++) { eeg_sample_t v=sample(i); while(!eeg_queue_push(&q,&v)) SwitchToThread(); }
    return 0;
}
#endif
int main(int argc, char **argv) {
    assert(eeg_crc32("123456789",9)==0xcbf43926u);
    ...
    assert(u32(s.pending+1016)==eeg_crc32(s.pending,1016));
    ...
    for(uint32_t i=0;i<200000;i++){while(!eeg_queue_peek(&q,&got))SwitchToThread();assert(got.sequence==i);assert(got.raw[0]==0xc0);eeg_queue_pop(&q);}
    assert(WaitForSingleObject(thread,10000)==WAIT_OBJECT_0);CloseHandle(thread);
#endif
    printf("PASS stream/CRC/queue/stop/backpressure/disconnect; sample=%u queue=%u stream=%u bytes\n",
